...
public enum InternalMethod {
    ...
    private static class InternalMethodMirrorType implements TypeMirror {
        ...
        public <R, P> R accept(TypeVisitor<R, P> v, P p) {
            var visitorGenericSuperclass = v.getClass().getGenericSuperclass();
            if (visitorGenericSuperclass instanceof ParameterizedType parameterizedType) {
                var actualTypeArgument = parameterizedType.getActualTypeArguments()[0];

                if (actualTypeArgument instanceof Class<?> typeClass && TypeName.class.isAssignableFrom(typeClass)) {
                    return result();
                }
            }
            throw new AutoRecordProcessorException("Cannot accept " + v + " visitor");
        }
